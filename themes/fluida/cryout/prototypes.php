...
function cryout_mobile_body_class( $classes ){
	$browser = ( ! empty( $_SERVER['HTTP_USER_AGENT']) ? sanitize_key( $_SERVER['HTTP_USER_AGENT'] ) : '');
...
function cryout_breadcrumbs(
    ...
    if ( is_category() ) {
		...
		// daily archive
		echo '<a href="' . esc_url( get_year_link( get_the_time( 'Y' ) ) ) . '">' . esc_html( get_the_time( 'Y' ) ) . '</a> ' . $separator . ' ';
		echo '<a href="' . esc_url( get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) ) . '">' . esc_html( get_the_time( 'F' ) ) . '</a> ' . $separator . ' ';
		...
		if ( get_post_type() != 'post' ) {
			...
			echo '<a href="' . esc_url( $homeUrl ) . '/' . $slug['slug'] . '/">' . $post_type->labels->singular_name . '</a>';
			if ( $showCurrent ) echo ' ' . $separator . ' ' . $before . esc_html( get_the_title() ) . $after;
		} else {
			...
			echo $cats;
			if ( $showCurrent ) echo $before . esc_html( get_the_title() ) . $after;
		}
