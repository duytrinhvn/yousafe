...
if (!function_exists('fluida_content_width')) :
	function fluida_content_width()
	{
		...
		switch ($options['fluida_sitelayout']) {
			...
			case '3cSr':
			case '3cSs':
				$content_width -= (int)$options['fluida_primarysidebar']; // primary sidebar
			case '2cSr':
			case '3cSl':
			case '3cSr':
			case '3cSs':
				$content_width -= (int)$options['fluida_secondarysidebar'];
				break; // secondary sidebar
		}
		...
		if (is_archive()) {
			switch ($options['fluida_magazinelayout']):
				case 2:
					$content_width = floor($content_width * 0.98 / 2);
					break; // magazine-two
				case 3:
					$content_width = floor($content_width * 0.96 / 3);
					break; // magazine-three
			endswitch;
		};
...
if (!function_exists('fluida_featured_width')) :
	function fluida_featured_width()
	{
		...
		switch ($options['fluida_sitelayout']) {
			...
			case '3cSr':
			case '3cSs':
				$width -= (int)$options['fluida_primarysidebar'] + $deviation; // primary sidebar
			case '2cSr':
			case '3cSl':
			case '3cSr':
			case '3cSs':
				$width -= (int)$options['fluida_secondarysidebar'] + $deviation;
				break; // secondary sidebar
		}
		...
		if (!is_singular()) {
			switch ($options['fluida_magazinelayout']):
				case 2:
					$width = ceil($width * 0.98 / 2);
					break; // magazine-two
				case 3:
					$width = ceil($width * 0.96 / 3);
					break; // magazine-three
			endswitch;
		};
...
if (!function_exists('fluida_header_image_url')) :
	function fluida_header_image_url()
	{
		...
		if (
			is_singular() && has_post_thumbnail($post->ID) && $fluida_fheader &&
...
if (!function_exists('fluida_header_image')) :
	function fluida_header_image()
	{
		...
		if (is_front_page() && function_exists('the_custom_header_markup') && has_header_video()) {
			...
			<img class="header-image" alt="<?php if (is_single()) the_title_attribute();
											elseif (is_archive()) echo esc_attr(get_the_archive_title());
											else echo esc_attr(get_bloginfo('name')) ?>" src="<?php echo esc_url($header_image) ?>" />
	...
	function fluida_get_layout_class()
	{
		...
		switch ($layout) {
			...
			case '2cSr':
				...
				break;
			case '3cSl':
				$class = "three-columns-left";
				break;
			case '3cSr':
				$class = "three-columns-right";
				break;
			case '3cSs':
...
function fluida_lpbox_width($options = array())
{
	...
	if ($options['fluida_lpboxlayout1'] == 1) {
		$totalwidth = $options['fluida_sitewidth'];
	} else {
		$totalwidth = $options['fluida_sitewidth'] - $options['fluida_primarysidebar'] - $options['fluida_secondarysidebar'];
	}
	$options['fluida_lpboxwidth1'] = intval($totalwidth / $options['fluida_lpboxrow1']);

	if ($options['fluida_lpboxlayout2'] == 1) {
		$totalwidth = $options['fluida_sitewidth'];
	} else {
		$totalwidth = $options['fluida_sitewidth'] - $options['fluida_primarysidebar'] - $options['fluida_secondarysidebar'];
	}
