...
jQuery(document).ready(function () {
  ...
  if (
    (cryout_theme_settings.fitvids == 2 &&
      cryout_theme_settings.is_mobile == 1) ||
...
function cryoutLpBoxesRatios() {
  for (index = 1; index <= cryout_theme_settings.lpboxratios.length; ++index) {
    jQuery(".lp-boxes-" + index + " .lp-box-image").keepRatio(
      cryout_theme_settings.lpboxratios[index - 1]
    );
  }
} /* cryoutLpBoxesRatios() */
...
function cryoutBoxesAnimation() {
  jQuery(".lp-boxes-animated.lp-boxes").mousedir(
    ".lp-boxes-animated.lp-boxes:not(.lp-boxes-animated2) .lp-box"
  );
}
...
function cryoutTitleLettering() {
  ...
  for (var i = 0, len = str.length; i < len; i++) {
    if (str[i] != " ") newstr += "<span>" + str[i] + "</span>";
...
function cryoutMenuAnimate() {
  jQuery("#access > .menu ul li > a:not(:only-child)").attr(
    "aria-haspopup",
    "true"
  ); /* IE10 mobile Fix */
...
function cryoutBackToTop() {
  jQuery(window).scroll(function () {
    ...
    if (jQuery(this).scrollTop() > 300) {
      ...
      jQuery(".fluida-fixed-menu #site-header-main").removeClass(
        "header-fixed"
      );
    }
  });
...
function cryoutInitNav(selector) {
  ...
  container
    .find(".page_item_has_children > a")
    .after('<button class="dropdown-toggle" aria-expanded="false"></button>');

  /* Toggle buttons and submenu items with active children menu items. */
  container
    .find(".current-menu-ancestor > button, .current-page-ancestor > button")
    .addClass("toggle-on");
  container
    .find(
      ".current-menu-ancestor > .sub-menu, .current-page-ancestor > .sub-menu, .current-menu-ancestor .children, .current-page-ancestor .children"
    )
    .show(0)
    .addClass("toggled-on");
...
jQuery.fn.mousedir = function (el) {
  ...
  $el.on("mouseenter", function () {
    jQuery(this).removeClass(
      "in-direction-left in-direction-right in-direction-up in-direction-down out-direction-left out-direction-right out-direction-up out-direction-down"
    );
...
function cryoutRemoveFocus(selector) {
  ...
  jQuery("input, textarea, select, button").on(
    "mouseup mousedown",
    function () {
...
function cryoutTabsWidget() {
  ...
  tabsNav.each(function () {
    ...
    localthis
      .next()
      .children(".cryout-wtab")
      .stop(true, true)
      .children("li")
      .hide()
      .parent()
      .siblings(localthis.find("a").attr("href"))
      .children("li")
      .show();
...
function cryoutPortfolioFilter() {
  jQuery("body").on("click", "#portfolio-filter > a", function (e) {
    ...
    jQuery("#portfolio-masonry .portfolio-entry")
      .each(function (i, elm) {
        if (filter == "all") {
...
function cryoutAutoScroll(document, history, location) {
  ...
  var anchorScrolls = {
    ANCHOR_REGEX: /^#[^ ]+$/,
    OFFSET_HEIGHT_PX:
      jQuery(".fluida-fixed-menu #site-header-main").height() + 40,
